...
import { COURSES, LIVE_CLASSES, SUBSCRIPTION } from '../../../../../route/route';
    ...
    useEffect(() => {
        if (token && slug) {
            const saveData = async () => {
                if (ProtectUrl(slug)) {
                    try {
                        let response = await axiosClient.post(`${webUrls.COURSE_DETAILS1_URL}/${slug}/`, {}, {
                            headers: {
                                            ...
                                            <div className="view-demo">
                                                <div className="alert alert-primary alert-dismissible solid alert-alt">
                                                    <div className="live-bg container">
                                                        <img src={liveGif} alt="gif" height={20} />
                                                    </div>
                                                    <div className="text-center mt-20">
                                                        <Link to={`${LIVE_CLASSES}/${item?.course?.slug}`} onClick={() => { sessionStorage.setItem("li_ve", JSON.stringify(item)) }}><img src={`${item?.thumbnail}`} alt="" height={150} /></Link>
                                                ...
                                                <div className="play-button text-center">
                                                    <Link to={`${LIVE_CLASSES}/${item?.course?.slug}`} onClick={() => { sessionStorage.setItem("li_ve", JSON.stringify(item)) }} className="popup-youtube">
                                        ...
                                        <div className="card-footer">
                                            <div className="courses-bx">
                                                <div className="dlab-info">
                                                    ...
                                                    <h5><Link to={`${LIVE_CLASSES}/${item?.course?.slug}`} onClick={() => { sessionStorage.setItem("li_ve", JSON.stringify(item)) }}>{item?.title}</Link>
                                                    </h5>
                                                </div>
                                            </div>
                                        </div>
