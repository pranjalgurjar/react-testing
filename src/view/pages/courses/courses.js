...
const Courses = (props) => {
    ...
    const EachPrefrence = (item) => {
        ...
        const filter = All_prefences?.find(dt => dt.slug === item.slug && dt?.is_active)
    ...
    useEffect(() => {
        ...
        setEachprefrence(couresPageData?.[0])
    }, [couresPageData])
                ...
                <div className="course-details-tab style-2">
                    <nav>
                        <div className="nav nav-tabs justify-content-start tab-auto" id="nav-tab" role="tablist">
                            {All_prefences && All_prefences?.map((item, index) => <button className={filterdata?.id === item?.id ? "nav-link active" : "nav-link"} id="nav-about-tab" onClick={() => { EachPrefrence(item) }} type="button" role="tab" key={index} >{item?.name}</button>
                            )}
                        </div>
                    </nav>
                    <div className="tab-content" id="nav-tabContent">
                        <div className="tab-pane fade active show" id="nav-about" role="tabpanel" aria-labelledby="nav-about-tab">
                            <div className="row mt-4">
                                {filterdata && filterdata?.courses?.map((item, index) => {
