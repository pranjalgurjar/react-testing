						...
						<div id="en-instruction" style={{ display: 'block' }}>
							...
							{(coupanData && coupanData.length ? coupanData : [])?.map((item, index) => <div key={index}><div className="ft-b mt-4" >
								...
								<div className="d-flex justify-content-between content align-items-center">
									...
									{coupans?.[0] === item.id ? <button className="btn btn-danger btn-sm" onClick={RemoveCop}>Remove Coupon</button> : <button className="btn btn-secondary btn-sm" onClick={() => totalPrice(item)}>Apply Coupon</button>}
								</div>
