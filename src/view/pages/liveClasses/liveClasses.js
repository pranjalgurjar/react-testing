...
const LiveClasses = () => {
  ...
  const courses = JSON.parse(localStorage.getItem("user_subscription"))
  const [value, setValue] = useState(courses[0]?.course?.slug)
  // remove duplicates from subscriptions
  var dps = {}
  const subscription_plan = courses?.filter((item) => {
  ...
  // for live classes
  const liveClasses = useCallback(async (slugs) => {
    try {
      setloading(true)
      let response = await axiosClient.get(`${webUrls.GET_LIVE_CLASSES_URL}/${slugs}/`, {
        headers: {
  ...
  }, [liveClasses, value, token, issubs])
      ...
      <div className="course-details-tab style-2">
        <TabContext value={value}>
          <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
            ...
            >
              {subscription_plan && subscription_plan?.map((item, index) => <Tab style={value === item?.course?.slug ? { color: "#886ffe" } : {}} label={item?.course?.name} value={(value === undefined) ? courses[0]?.course?.slug : item?.course?.slug} key={index} />)}
            </TabList>
          </Box>
