...
const CoursedetailsTwo = () => {
    ...
    const searchFun = (e) => {
        ...
        let data = getSearchData?.[0]?.CategoryLectures?.filter(item => item?.title?.toLowerCase().includes(search.toLowerCase()))
        if (data?.length) {
                                    ...
                                    <div className="tab-content" id="nav-tabContent">
                                        ...
                                        <div className={testSeriesTab ? "tab-pane fade show active" : "tab-pane fade"} id="nav-testseries" role="tabpanel" aria-labelledby="nav-testseries-tab">
                                            {(details?.TestSeries?.length) ? <>
                                                {(searchData && searchData.length ? searchData : details?.TestSeries).map((courseTest, index) => {
                                                    return (
                                                        <>
