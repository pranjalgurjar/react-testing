...
const LecturesData = ()=>{
    setGetSearchData(details?.Lectures)
    setSearchData([])
...
const notesData = ()=>{
    setGetSearchData(details?.Notes)
    setSearchData([])
...
const testSeriesData = ()=>{
    setGetSearchData(details?.TestSeries)
    setSearchData([])
    setVideolectureTab(false)
    setPdfNotesTab(false)
...
const searchFun = (e)=>{
  ...
  let data =  getSearchData?.filter(item=>item?.topic?.name?.toLowerCase().includes(search.toLowerCase()))
  setSearchData(data)
}
// console.log(searchData,"sea");
    return (<>
        {(details && details?.Lectures?.length) ? <>
                                ...
                                <div className="course-details-tab style-2">
                                            ...
                                            <div className="col-xl-8">
                                                <div className="nav nav-tabs justify-content-start tab-auto" id="nav-tab" role="tablist">
                                                    <button className={videoLecturesTab?"nav-link active":"nav-link"} id="nav-videos-tab" onClick={LecturesData} type="button" role="tab" >
                                                        <i className="fa fa-video-camera me-2" /> Video Lectures ({details?.Lectures?.length}) </button>
                                                    <button className={pdfNotesTab?"nav-link active":"nav-link"} id="nav-pdfnotes-tab"  onClick={notesData} type="button" role="tab" >
                                                        <i className="fa fa-book me-2" /> PDF Notes ({details?.Notes?.length}) </button>
                                                    <button className={testSeriesTab?"nav-link active":"nav-link"} id="nav-testseries-tab" onClick={testSeriesData} type="button" role="tab" >
                                                        <i className="bi-journal-text me-2" /> Test Series ({details?.TestSeries?.length}) </button>
                                                </div>
                                            </div>
                                    ...
                                    <div className="tab-content" id="nav-tabContent">
                                        <div className={videoLecturesTab?"tab-pane fade show active":"tab-pane fade"} id="nav-videos" role="tabpanel" aria-labelledby="nav-videos-tab">
                                            {(searchData && searchData.length?searchData:details?.Lectures)?.map((coursedetail, index) => {
                                                return (
                                                    <div className="about-content" key={index}>
                                                        ...
                                                        <div className="row">
                                                            {(coursedetail?.CategoryLectures)?.map((data, index) => {
                                                                return (
                                            ...
                                            {(searchData && searchData.length?searchData:details?.Notes)?.map((coursePdf, index) => {
                                                return (
                                                    <div className="about-content" key={index}>
                                                        ...
                                                        <div className="row">
                                                            {coursePdf?.CategoryNotes?.map((topicData, index) => {
                                                                return (
                                        ...
                                        <div className={testSeriesTab?"tab-pane fade show active":"tab-pane fade"} id="nav-testseries" role="tabpanel" aria-labelledby="nav-testseries-tab">
                                            {(searchData && searchData.length?searchData:details?.TestSeries)?.length ? <> {details?.TestSeries?.map((courseTest, index) => {
                                                return (
                                                    <>
                                                        ...
                                                        <div className="row mt-4" key={index}>
                                                            {courseTest?.CategoryTestSeries?.map((testdata, index) => {
                                                                return (
                                                                    <div className="col-xl-3 col-sm-3" key={index}>
                                                                        <div className="card">
                                                                            <div className="card-body">
